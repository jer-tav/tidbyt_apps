...
def main(config):
    ...
    if resp.status_code != 200:
        fail("Failed to get the current player summary with code {}".format(resp.status_code))

    if len(resp.json()["response"]["players"]) != 1:
        fail("Failed to find player with SteamID {}".format(steam_id))

    if resp.json()["response"]["players"][0]["communityvisibilitystate"] != 3:
        fail("Profile is not public, can't get current game")
    ...
    if "gameextrainfo" in resp.json()["response"]["players"][0].keys():
        ...
        if resp.status_code != 200:
            fail("Failed to get the current game icon with code {}".format(resp.status_code))

        if resp.json()["response"]["game_count"] != 1:
            fail("Failed to find game with ID {}".format(current_game_id))
        ...
        # Should probably do some sanity checking here.
        ...
        if resp.status_code != 200:
            fail("Failed to get the current player's previously played games with code {}".format(resp.status_code))

        if (resp.json()["response"]["total_count"] == 0):
            fail("Nothing to render - no previous games")
